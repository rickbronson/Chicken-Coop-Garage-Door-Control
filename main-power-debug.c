...
#else
#ifdef POWER_DEBUG
#define AWU_TBR_TIMEBASE AWU_TBR_TIMEBASE_1S
#define AWU_APR_TIMEBASE AWU_APR_TIMEBASE_1S
#else
...
#define BAUD_DIV ((((HSICLOCKFREQ * 2) / BAUDRATE) + 1) / 2)
...
INTERRUPT_HANDLER(AWU_IRQHandler, 1)
  ...
  /* Clear AWU peripheral pending bit */
	...
	if (p_data->got_60Hz_int)
		...
		{
#if defined(SERIAL_DEBUG) || defined(POWER_DEBUG)
		LED_PORT->ODR ^= LED_PIN;  /* toggle LED */
#else
		LED_PORT->ODR |= LED_PIN;  /* turn LED off */
#endif
		if (++p_data->sec_30_int >= 2)  /* 1 min? */
			{
			p_data->sec_30_int = 0;
			if (++p_data->mins_int >= MINS_PER_HR)  /* 1 min? */
				p_data->mins_int = 0;
...
void gpio_init(void)
  ...
  /* Initialize LED pins in Output Mode */
	...
	DOOR_SENSE_PORT->CR1 |= DOOR_SENSE_PIN;  /* apply pull up */
...
#ifdef SERIAL_DEBUG
		while (UART1->CR2 & UART1_CR2_TCIEN)  /* debug stay here until sent before going into halt */
			;
#endif
